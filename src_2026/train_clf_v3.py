"""
Training script v3 — ResNet1D on beat-level amplitude features.

Input : (MAX_BEATS, 6) R-peak amplitude sequence per 30s window
Output: 4-class movement label

Treats the beat sequence as a 1D signal with 6 channels and MAX_BEATS timesteps.
ResNet1D from resnet1d.py is agnostic to sequence length (AdaptiveAvgPool at end).
"""
...
MODEL_SAVE_PATH = '../models/movement_clf_v3.pth'
HISTORY_PATH    = '../models/movement_clf_v3_history.json'
WEIGHTS_PATH    = '../models/movement_clf_v3_weights.json'
LOG_DIR         = '../logs'

LEARNING_RATE   = 1e-4
WEIGHT_DECAY    = 1e-3      # stronger regularisation
BATCH_SIZE      = 128       # beat sequences are tiny — fits large BS
MAX_EPOCHS      = 150
PATIENCE        = 20
VAL_SPLIT       = 0.15
NUM_WORKERS     = 2
...
IN_CHANNELS     = 6         # beat amplitude channels
N_CLASSES       = 4
...
def run_epoch(model, loader, criterion, optimizer, device, train,
    ...
    with ctx:
        for bi, (x, mask, y) in enumerate(loader):
            # ResNet1D expects (B, C, L) — transpose beat dim and channel dim
...
def main():
    ...
    # ResNet1D on beat sequences: input (B, 6, MAX_BEATS=60)
    model    = ResNet1D(in_channels=IN_CHANNELS, n_classes=N_CLASSES,
                        base_ch=16).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    print(f'Model: ResNet1D (base_ch=16, beat input)  params={n_params/1e3:.1f}K')
