"""
Training script v7 — ResNet1D pe beat features CORECTE.

Fix-uri fata de v3/v5:
  1. Ferestre 5s in loc de 30s  → 85% din ferestre sunt pure (o singura clasa)
     vs 31% la 30s → elimina label noise masiv
  2. Filtru puritate 99%         → aruncam ferestrele de tranzitie
  3. Zero-mean fara unit-std     → std-ul rezidual e semnalul:
       stationar ≈ 0, miscare > 0
     (per-window unit-std distrugea tocmai aceasta informatie)
  4. Cache weights cu nou key (v7) — distributia claselor se schimba
     cu filtrul de puritate
"""
...
MODEL_SAVE_PATH = '../models/movement_clf_v7.pth'
HISTORY_PATH    = '../models/movement_clf_v7_history.json'
WEIGHTS_PATH    = '../models/movement_clf_v7_weights.json'
LOG_DIR         = '../logs'

LEARNING_RATE   = 1e-4
WEIGHT_DECAY    = 1e-3
BATCH_SIZE      = 256       # mai multe ferestre → BS mai mare
MAX_EPOCHS      = 100
PATIENCE        = 15
VAL_SPLIT       = 0.15
NUM_WORKERS     = 4
...
def run_epoch(model, loader, criterion, optimizer, device, train,
    ...
    with ctx:
        for bi, (x, mask, y) in enumerate(loader):
            x = x.permute(0, 2, 1).to(device)   # (B, 6, MAX_BEATS)
            y = y.to(device)
...
def main():
    ...
    full_dataset = BeatDataset(
        ...
        normalize_std=False,    # zero-mean only — std rezidual e semnalul
    )
    ...
    model    = ResNet1D(in_channels=IN_CHANNELS, n_classes=N_CLASSES,
                        base_ch=16, dropout=DROPOUT).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    print(f'Model: ResNet1D (base_ch=16, dropout={DROPOUT})  params={n_params/1e3:.1f}K')
