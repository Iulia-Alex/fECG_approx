"""
Training script v4 — BeatTransformer on beat-level amplitude features.

Same dataset as v3 (BeatDataset, 30s windows, ~40 beats).
Uses the padding mask for proper attention — padded positions are ignored.
"""
...
MODEL_SAVE_PATH = '../models/movement_clf_v4.pth'
HISTORY_PATH    = '../models/movement_clf_v4_history.json'
WEIGHTS_PATH    = '../models/movement_clf_v3_weights.json'   # reuse v3 weights cache
LOG_DIR         = '../logs'

LEARNING_RATE   = 3e-4
WEIGHT_DECAY    = 1e-3
BATCH_SIZE      = 256       # Transformer is very light
MAX_EPOCHS      = 150
PATIENCE        = 20
VAL_SPLIT       = 0.15
NUM_WORKERS     = 2
...
def run_epoch(model, loader, criterion, optimizer, device, train,
    ...
    with ctx:
        for bi, (x, mask, y) in enumerate(loader):
            ...
            mask = mask.to(device)    # (B, MAX_BEATS) bool
            y    = y.to(device)
...
def main():
    ...
    model    = BeatTransformer(n_channels=6, d_model=64, n_heads=4,
                               n_layers=4, max_beats=60,
                               n_classes=N_CLASSES, dropout=0.2).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    print(f'Model: BeatTransformer  params={n_params/1e3:.1f}K')
