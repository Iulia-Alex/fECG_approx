"""
Training script v9 — MLP pe features per-faza completa.

Insight cheie: fazele complete (12-83s) contin pattern-ul intreg de miscare.
Ferestrele fixe (5-15s) vedeau doar un fragment → indistinguibile.
RF pe faze complete: 92.2% val accuracy.

Features (36): [std, slope, R², fft_amp, fft_freq, lag5_autocorr] x 6 canale
Arhitectura: FC(36→256→256→128→4) cu BN + Dropout
"""
...
MODEL_SAVE_PATH = '../models/movement_clf_v9.pth'
HISTORY_PATH    = '../models/movement_clf_v9_history.json'
LOG_DIR         = '../logs'
...
class PhaseMLP(nn.Module):
    def __init__(self, n_features=36, n_classes=4, dropout=0.3):
        super().__init__()
...
def main():
    ...
    print(f'\nLoading PhaseDataset from {DATA_DIR} ...')
    full_dataset = PhaseDataset(DATA_DIR)
    ...
    print(f'Model: PhaseMLP (36→256→256→128→4)  params={n_params/1e3:.1f}K')
