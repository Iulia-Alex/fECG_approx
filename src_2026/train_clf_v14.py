"""
Training script v14 — RF + MLP + Transformer pe 71 features (spectrale + inter-canal).

Features v14 (71 total):
  36 spectrale originale (std, slope, R², FFT amp, FFT freq, lag5) x 6 canale
  15 cross-correlatie inter-canal la lag 0 (15 perechi)
  15 cross-correlatie inter-canal la lag 5 (15 perechi)
   5 PCA pe (N_beats, 6): var PC1, var PC2, var PC3, ratio PC1/PC2, norm PC1

Modele antrenate in acelasi script:
  1. RandomForest (sklearn)
  2. MLP (PyTorch) — Linear(71->256->256->128->4)
  3. Transformer (PyTorch) — features ca tokens, self-attention
"""
...
from clf_dataset import ClfDatasetV14, N_CLASSES

DATA_DIR           = '../data/movement_ecg'
MODEL_MLP_PATH     = '../models/movement_clf_v14_mlp.pth'
MODEL_TRANS_PATH   = '../models/movement_clf_v14_transformer.pth'
HISTORY_PATH       = '../models/movement_clf_v14_history.json'
LOG_DIR            = '../logs'
...
# ---------------------------------------------------------------------------
# Transformer — trateaza fiecare feature ca un token scalar
# Alternativ: grupeaza features in blocuri per canal (6 canale x 11-12 features)
# ---------------------------------------------------------------------------
class FeatureTransformer(nn.Module):
    """
    Trateaza cele 71 features ca o secventa de 71 tokens scalari.
    Fiecare scalar e proiectat la d_model dim.
    """
    def __init__(self, n_features=N_FEATURES, d_model=64, nhead=4,
                 num_layers=3, n_classes=4, dropout=0.3):
        super().__init__()
        self.proj    = nn.Linear(1, d_model)
        # positional embedding invatabil (71 pozitii)
    ...
    def forward(self, x):
        # x: (B, 71) -> (B, 71, 1) -> proiectie -> (B, 71, d_model)
        B = x.size(0)
        x = x.unsqueeze(-1)                               # (B, 71, 1)
        x = self.proj(x)                                  # (B, 71, d_model)
        pos = torch.arange(self.n_features, device=x.device)
        x   = x + self.pos_emb(pos)                       # + positional
        x   = self.encoder(x)                             # (B, 71, d_model)
...
def train_model(model, name, save_path, train_loader, val_loader,
    ...
    print(f'\n{"="*60}')
    print(f'Training {name}  params={n_params/1e3:.1f}K')
...
def main():
    ...
    print(f'\nLoading ClfDatasetV14 from {DATA_DIR} ...')
    full_ds = ClfDatasetV14(DATA_DIR)
    ...
    # -----------------------------------------------------------------------
    print('\n' + '='*60)
    print('Training Random Forest (500 trees) on 71 features ...')
    X_tr = full_ds.X_norm[train_idx]
    y_tr = full_ds.y[train_idx]
    X_va = full_ds.X_norm[val_idx]
    y_va = full_ds.y[val_idx]
    ...
    names = (
        ...
        [f'xcorr0_ch{i}{j}' for i in range(6) for j in range(i+1, 6)] +
        [f'xcorr5_ch{i}{j}' for i in range(6) for j in range(i+1, 6)] +
        ['pca_var1','pca_var2','pca_var3','pca_ratio','pca_norm1']
    )
    ...
    mlp       = PhaseMLP(n_in=N_FEATURES, n_classes=N_CLASSES,
    ...
    # -----------------------------------------------------------------------
    print(f'\n{"="*60}')
    print(f'REZULTATE FINALE v14 (71 features: spectrale + inter-canal):')
    print(f'  RF          val acc: {rf_val_acc:.2f}%')
