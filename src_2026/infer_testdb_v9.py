"""
Inference on Test_DB — v9 model (ComplexUNetV9, soft mask + combined loss).
Plots: ground truth (verde) vs v9 (portocaliu).
"""
...
for fname in files:
    ...
    for i, ch in enumerate(CHANNELS):
        ax = axes[i]
        ax.plot(t, fecg_norm[ch], color='seagreen',  lw=1.2, label='Ground truth', alpha=0.9)
        ax.plot(t, time_v9[ch],   color='darkorange', lw=0.9, label=f'v9 ep{ep_v9} (mask)', alpha=0.85)
    ...
    fig.suptitle(
        f'fECG Extraction — v9 ComplexUNetV9 mask+MSE+CplxMSE (ep {ep_v9}) — {stem}\n'
        f'Window: {START//FS}–{(START+WINDOW)//FS} s',
        fontsize=11,
    )
    plt.tight_layout()
    out_path = os.path.join(OUT_DIR, f'{stem}_v9_ep{ep_v9}.png')
