"""
Training script v1 — ResNet1D for fetal movement classification.

Input : ground-truth fECG (6, 4000) from .mat files
Output: 4-class movement label (0=stationary, 1=linear, 2=helical, 3=screw)

Design choices:
  - ResNet1D (~1.5 M params): lightweight enough for BS=64 on M4000 8GB
  - CrossEntropyLoss with inverse-frequency class weights (class 0 = 53% -> downweighted)
...
MODEL_SAVE_PATH  = '../models/movement_clf_v1.pth'
HISTORY_PATH     = '../models/movement_clf_v1_history.json'
WEIGHTS_PATH     = '../models/movement_clf_v1_weights.json'
LOG_DIR          = '../logs'
...
def main():
    ...
    full_dataset = MovementClfDataset(DATA_DIR)
    # File-level split — all windows from a recording go entirely to train or val
    # Prevents data leakage between similar windows from the same file
