...
DILATION = 30   # ±30 samples = ±30ms în jurul fiecărui R-peak
...
def _load_fqrs(mat_path):
    """Incarca fqrs din .mat sau din cache .npz."""
...
def _make_peak_mask(fqrs, start, window, dilation=DILATION):
    """Creeaza un peak mask de forma (window,) cu 1.0 langa fiecare R-peak."""
    mask = np.zeros(window, dtype=np.float32)
    # Selectam peak-urile din fereastra curenta
...
class MovementECGDatasetV10(MovementECGDataset):
    ...
    def __getitem__(self, idx):
        ...
        # Trebuie sa stim si file_idx si start pentru peak_mask
