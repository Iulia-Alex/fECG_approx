"""
Inference on Test_DB — v8 model (ComplexUNetV7 + MSE loss).
Plots ground truth vs predicted fECG for each test file, 3 channels.
"""
...
MODEL_V8  = '/shared_storage/iulia.orvas/paper/fECG_approx/models/movement_CUNet_v8_mse.pth'
HIST_V8   = '/shared_storage/iulia.orvas/paper/fECG_approx/models/movement_CUNet_v8_mse_history.json'
...
model_v8 = load_model(MODEL_V8)
ep_v8    = best_epoch(HIST_V8)
print(f'v8 loaded (best ep {ep_v8})')
...
for fname in files:
    ...
    pred_time = spec_to_time(pred_v8)
    ...
    for i, ch in enumerate(CHANNELS):
        ax = axes[i]
        ax.plot(t, fecg_norm[ch],  color='seagreen',   lw=1.0, label='Ground truth fECG', alpha=0.9)
        ax.plot(t, pred_time[ch],  color='darkorange',  lw=0.9, label='v8 predicted',      alpha=0.85)
    ...
    fig.suptitle(
        f'fECG Extraction — v8 ComplexUNet+MSE (ep {ep_v8}) — {stem}\n'
        f'Window: {START//FS}–{(START+WINDOW)//FS} s  |  normalised amplitude',
        fontsize=11,
    )
    plt.tight_layout()
    out_path = os.path.join(OUT_DIR, f'{stem}_v8_ep{ep_v8}.png')
