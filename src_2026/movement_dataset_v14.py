"""
Dataset v14 — identic cu movement_dataset.py dar cu:
  - Downsample 1000Hz → 500Hz (factor 2, subsampling simplu)
  - STFT params din paper: n_fft=256, hop=15, win=100 → natural 129×128
  - Resize la 128×128 (drop 1 freq bin, exact ca paper)
  - Fereastră = 3830 samples @ 1000Hz = 1915 @ 500Hz = 3.83s
  - fecg_time la 500Hz (1915 samples), pentru iSTFT în loss

Compatibil cu signal_mse din train_movement_v14.py:
  ORIG_F=129, ORIG_T=128, iSTFT cu n_fft=256, hop=15, win=100, length=1915.
"""
...
# Parametri dataset
FS_ORIG         = 1000     # FS original al înregistrărilor
DOWNSAMPLE      = 2        # factor downsample: 1000 → 500 Hz
FS              = 500      # FS după downsample
WINDOW_ORIG     = 3830     # fereastră @ 1000 Hz (= 3.83 s)
WINDOW_FS       = 1915     # fereastră @ 500 Hz  (= 3.83 s)
STRIDE_ORIG     = 3830     # stride @ 1000 Hz (no overlap)
N_SAMPLES_TOTAL = 600_000  # lungimea totală a fiecărui fișier @ 1000 Hz
...
class MovementECGDatasetV14(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Normalizare per canal cu std-ul amestecului
        ...
        # fecg_time @ 500 Hz (pentru SignalMSE)
