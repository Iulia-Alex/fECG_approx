"""
Overlay plot v3 — predicted fECG vs ground truth.
v1 architecture + peak-weighted loss (128×400, 1000Hz). Runs on CPU.
"""
...
MODEL_PATH  = '../models/movement_CUNet_128x400_peakw.pth'
...
OUT_PATH = f'../plots_2026/inference_overlay_v3_ep{EPOCH}.png'
...
fig.suptitle(
    f'fECG Extraction v3 (peak-weighted loss) — Predicted vs Ground Truth  (epoch {EPOCH})\n'
    f'{os.path.basename(test_file)}   |   Window: {START // FS}–{(START + WINDOW) // FS} s   '
    f'(SignalMSE + 5×PeakMSE + 0.1×ComplexMSE)',
    fontsize=11
)
plt.tight_layout()
