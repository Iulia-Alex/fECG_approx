"""
Training script v8 — MLP pe beat features aplatizate.

Problema cu ResNet (v3-v7):
  ResNet face downsampling 20→10→5→3→2→1 timestep.
  La output, raman cu o singura pozitie → global average pooling = 0
  pentru toate clasele (zero-mean). Structura temporala e distrusa complet.

Solutia:
  1. MLP direct pe (MAX_BEATS * 6) features aplatizate — pastreaza
     ordinea temporala (beat 1 e altceva decat beat 20)
  2. Ferestre 15s cu filtru puritate 90% — suficient pentru a vedea
     trenduri (linear/screw) si oscilatii (helical)
  3. Zero-mean fara unit-std — std rezidual e semnalul

MLP architecture:
  120/180 → 512 → 256 → 128 → 4
  BatchNorm + Dropout(0.3) la fiecare strat ascuns
"""
...
MODEL_SAVE_PATH = '../models/movement_clf_v8.pth'
HISTORY_PATH    = '../models/movement_clf_v8_history.json'
WEIGHTS_PATH    = '../models/movement_clf_v8_weights.json'
LOG_DIR         = '../logs'

WINDOW_SEC      = 15        # suficient sa vada trend linear/helical
STRIDE_SEC      = 7.5
MAX_BEATS       = 40        # la 160bpm, 15s ≈ 40 beats
PURITY          = 0.90      # 90% puritate → mai multe ferestre
...
# ---------------------------------------------------------------------------
class BeatMLP(nn.Module):
    """MLP pe beat amplitudini aplatizate — pastreaza ordinea temporala."""

    def __init__(self, max_beats=40, n_channels=6, n_classes=4, dropout=0.3):
        super().__init__()
        in_dim = max_beats * n_channels
    ...
    def forward(self, x):
        # x: (B, MAX_BEATS, 6) — aplatizat la (B, MAX_BEATS*6)
...
def main():
    ...
    print(f'\nLoading dataset (window={WINDOW_SEC}s, purity={PURITY:.0%}) ...')
    full_dataset = BeatDataset(
        DATA_DIR,
    ...
    print(f'Pure windows: {len(full_dataset)}  '
          f'(train={len(train_set)}, val={len(val_set)})')
    print(f'File-level split: ~{int(len(full_dataset.files)*(1-VAL_SPLIT))} train, '
    ...
    model    = BeatMLP(max_beats=MAX_BEATS, n_channels=6,
                       n_classes=N_CLASSES, dropout=DROPOUT).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    print(f'Model: BeatMLP ({MAX_BEATS}×6→512→256→128→4)  params={n_params/1e3:.1f}K')
