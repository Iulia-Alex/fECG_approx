...
# ---------------------------------------------------------------------------
# Activations (identice cu v15 / paper)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Down/Up sampling (identice cu v15)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Attention Gate (NOVITATE v16)
#
# Inspiratie: Schlemper et al. 2019 "Attention U-Net"
#   https://github.com/ozan-oktay/Attention-Gated-Networks
#
# Adaptare complex: α calculat pe magnitudini |skip| si |gate|,
# aplicat ca scalar real pe semnalul complex skip:
#   attended_x = α(|skip|, |gate|) ⊙ skip
#
# Motivatie:
#   - |z| = amplitudinea (cat de puternica e trasatura)
#   - atentia decide daca amplitudinea e utila, nu care e faza
#   - faza (morfologia QRS, directia) e pastrata nemodificata
# ---------------------------------------------------------------------------
class ComplexAttentionGate(nn.Module):
    """
    Additive attention gate pentru complex features.
    skip si gate sunt la aceeasi rezolutie spatiala.

    Args:
        F_x   : nr. canale skip connection (encoder)
        F_g   : nr. canale gating signal (decoder)
        F_int : nr. canale intermediare (de obicei max(F_x,F_g)//2)
    """
    def __init__(self, F_x: int, F_g: int, F_int: int):
        super().__init__()
        # θ_x: proiecteaza magnitudinea skip-ului pe spatiul intermediar
        self.theta_x = nn.Sequential(
            nn.Conv2d(F_x, F_int, kernel_size=1, bias=False),
            nn.BatchNorm2d(F_int),
        )
        # θ_g: proiecteaza magnitudinea gating signal pe spatiul intermediar
        self.theta_g = nn.Sequential(
            nn.Conv2d(F_g, F_int, kernel_size=1, bias=False),
            nn.BatchNorm2d(F_int),
        )
        # ψ: coeficient de atentie in [0,1] per spatiu (1 canal — channel-agnostic)
    ...
    def forward(self, x_skip: torch.Tensor, x_gate: torch.Tensor) -> torch.Tensor:
        """
        x_skip : complex (B, F_x, H, W) — skip din encoder
        x_gate : complex (B, F_g, H, W) — gating signal din decoder (aceeasi rezolutie)
        Returns: complex (B, F_x, H, W) — attended skip
        """
        # Magnitudini reale pentru calculul atentiei
        mag_s = torch.abs(x_skip)   # (B, F_x, H, W)
        mag_g = torch.abs(x_gate)   # (B, F_g, H, W)

        # Proiectii independente + combinare aditionala (Eq. din paper)
        ...
        # Aplica atentia pe semnalul complex — broadcast pe canale
        return alpha * x_skip
...
class ComplexAttentionUNet(nn.Module):
    ...
    def __init__(self, dimension: int, in_channels: int = 6):
        ...
        # AG3: skip=res3 (256ch), gate=bottleneck (256ch), intermediar=128
        self.ag3 = ComplexAttentionGate(F_x=256, F_g=256, F_int=128)
        # AG2: skip=res2 (128ch), gate=up1_out (128ch), intermediar=64
        self.ag2 = ComplexAttentionGate(F_x=128, F_g=128, F_int=64)
        # AG1: skip=res1 (64ch),  gate=up2_out (64ch),  intermediar=32
        self.ag1 = ComplexAttentionGate(F_x=64,  F_g=64,  F_int=32)
        # ────────────────────────────────────────────────────────────────────

        # Decoder (identic cu v15 ca structura, dar primeste attended skip)
