"""
Resume training v1 (ComplexUNet 0.59M, SignalMSE) de la epoch 200.

Incarca checkpoint + history existente si continua pana la MAX_EPOCHS=400
(sau pana se triggereaza early stopping cu patience=20).

Salveaza in acelasi fisiere ca v1 original:
...
WEIGHT_DECAY    = 1e-5
BATCH_SIZE      = 32
MAX_EPOCHS      = 400     # extins de la 200
PATIENCE        = 20      # crescut de la 15 — mai rabdator
VAL_SPLIT       = 0.15
NUM_WORKERS     = 2
PRINT_EVERY     = 20
SEED            = 42
...
def main():
    ...
    if os.path.exists(HISTORY_PATH) and os.path.exists(MODEL_SAVE_PATH):
        ...
        print(f'Resumed de la epoca {epochs_done}, '
              f'best val={early_stop.best_loss:.6f} @ ep {early_stop.best_epoch + 1}')
    else:
        print('ATENTIE: nu s-a gasit checkpoint, se incepe de la zero!')
